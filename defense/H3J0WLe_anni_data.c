...
double Logfnow=1.0, Logfmin=0.00001, MinStateLogg;
...
int main(int argc, char *argv[]){
  ...
  for(i=0; i<StatesLength; i++){
    ...
    char anni_data_f[] = "anni_data_jam_f";
    char anni_data_H[] = "anni_data_jam_H";
    FILE *ff, *fH;
    ff=fopen(anni_data_f,"w");
    fH=fopen(anni_data_H,"w");
    for(i=0; i<MaxStateFound; i++){
        state[i].logg=0.0;
		state[i].H=0.0;
  	}
	Total_Steps=0.0;
	Inner_Loop_Steps=0.0;
  	while(Logfnow > Logfmin){
		...
		//if( ((log2(Total_Steps)-(int)log2(Total_Steps))<1.0e-12) || (Total_Steps<10.) ){
			fprintf(ff, "%.2g,", Logfnow);
			fprintf(fH, "%.2g,", Logfnow);
			
			for(i=0; i<MaxStateFound; i++){
				fprintf(ff, "%.4lf", state[i].logg);
				fprintf(fH, "%.0f", state[i].H);
				if(i==MaxStateFound-1){
					fprintf(ff, "\n");
					fprintf(fH, "\n");
				} else {				
					fprintf(ff, ",");
