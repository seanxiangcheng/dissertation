...
Here the RG setup is extended to a more general case by including $H$. The procedure of such RG has been developed by Brunson and Boettcher in FM of Hanoi networks \cite{brunson2014rg}. In AFM, the same procedure is followed with corresponding adjustments of temperature $T$ ($\mu$ in RG) and prefactors of these derivations. Instead of detailed derivations \cite{brunson2014rg}, only the general logic is described here. 
...
The chaotic transition temperatures can be determined from the eigenvalues of the Jacobian matrix.
...
\begin{figure}
\centering \includegraphics[width=0.7\columnwidth]{Chapter-3/HN35P6DOS_h2}
\protect\caption{Density of states from Wang-Landau sampling for system size $N=512$. Both the planar networks have highly degenerated ground states; while the nonplanar ones have unique ground states. }
\label{fig:afm-doswl} 
\end{figure}
...
One of the main goals of RG is to explore the chaotic behaviors found in the fixed point analysis. In AFM, the free energy with different boundary conditions has been used to study the chaos in physical systems. 

\begin{figure}
\centering \includegraphics[width=0.8\columnwidth]{Chapter-3/freeE_lowT2highT.pdf}
\protect\caption{The free energy difference between the 2 boundaries conditions at different temperatures. The system size is $N=2^k$, which are $2^16$, $2^24$, $2^32$ in the figure above. Obviously, at higher temperatures, there is no difference; at low temperatures, there is more and more difference variations for bigger and bigger system sizes. A zoomed-in plot for low temperatures is shown next (Fig. \ref{fig:afm-freeEdiff}).  }
\label{fig:afm-freeEdiff_allT} 
\end{figure}

\begin{figure}
\centering \includegraphics[width=0.8\columnwidth]{Chapter-3/freeEdifference_k16_32.pdf}
\protect\caption{The free energy difference between the 2 boundaries conditions at low temperatures $T<2.0$. }
\label{fig:afm-freeEdiff} 
\end{figure}


\begin{figure}
\centering \includegraphics[width=0.8\columnwidth]{Chapter-3/chaotic_exponents_freeE.pdf}
\protect\caption{Number of free energies crossings fitted to $k$ (=$\log_2 N$) using the equation $N_c = A N^\alpha$. Two methods are used to find the exponents. Method 1 (left figure): $\log_2 N_c = \log_2 A + \alpha \log_2 N= \log_2 A + \alpha k$; method 2 (right figure): $\log_2 N_c / \log_2 N = \log_2 A / \log_2 N + \alpha$ which is $\log_2 N_c / k = \log_2 A / k + \alpha$. The exponent $\alpha$ from method 1 and method 2 are $0.29\pm 0.01$ and $0.30\pm 0.01$, respectively. }
