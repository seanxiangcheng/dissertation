...
N_c = A N^\alpha
\end{equation}
where $A$ is a constant, $N$ is the system size, and $\alpha$ is the exponent. The exponent $\alpha$ is called chaotic exponent. 

\begin{figure}
\centering \includegraphics[width=0.8\columnwidth]{Chapter-3/chaotic_exponents_freeE_binning.pdf}
\protect\caption{Number of free energies crossings fitted to $k$ (=$\log_2 N$) in temperature range $T \in [10^{-3}, T_g]$. Two methods of fitting are used: Eq. \ref{eq:afm-fit1} (left figure) and Eq. \ref{eq:afm-fit2} (right figure). The exponents are $0.319\pm 0.006$ and $0.321 \pm 0.009$, respectively. Both of the $R^2$ is bigger than $0.98$.}
\label{fig:afm-freeEfit} 
\end{figure}


There is two way to obtain chaotic exponent $\alpha$. The first way is 
\begin{equation}
...
\label{eq:afm-fit2}
\end{equation}
where $\log_2 N = k$. These two ways can show us the trend from different perspectives as shown in Fig. \ref{fig:afm-freeEfit}. 
These two exponents are $0.319\pm 0.006$ and $0.321 \pm 0.009$, respectively. Both strongly show there is a polynomial increase with system size. 
